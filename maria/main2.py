...
def main():
    ...
    for plate2 in maria2.startWatchingaBack():
            ...
            if i == 5:
                break
    result = maria.sendRecognizedPlate(datos2[2]);
    ...
    if result != 0:
        ...
        print("Cerrar puerta")
            door.CloseDoor()
        plate = datos[2]
        r = requests.post("http://192.168.0.10:80/maria/ChangeState0/?username=BenjaminGuzman&password=MariaUribe&plate={}".format(plate), auth=("BenjaminGuzman", "MariaUribe"))
            print(r.status_code)
